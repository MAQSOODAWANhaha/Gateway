...
- 所有沟通必须使用中文。
- 文档必须使用中文。
- 文档只保存在 docs/ 目录下（根目录 README.md 作为项目入口说明除外）。
- 新增文件尽量使用 ASCII，若必须使用中文或其它字符则说明理由。
- 代码与文档变更保持一致。
- 任何功能实现、接口变更、默认值/行为调整：必须同步更新对应文档（docs/ 与必要时 README.md），不得出现“只改代码不改文档”。
- 模块保持小而清晰，避免隐藏副作用。
- 配置校验要明确、错误信息要清晰。

## 工作流约束（重要）
- 未经用户在当前对话中明确同意：不得执行 `git commit`、`git push`、`git reset`、`git revert` 等会改写提交历史或推送远端的操作。
- 如需要提交/推送：必须先说明将提交的内容与提交信息，并等待用户确认后再执行。
